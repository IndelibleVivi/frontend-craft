...
Everything happens in the editor's own toolbar:

| Action | What it actually does |
| --- | --- |
| **Save** | Writes the current title and body to this browser. |
| **Reopen** | Loads the last stored copy back into the editor — the way to check what was really kept. |
| **Discard draft** | Returns the editor to the last saved copy. Nothing is written and nothing is deleted; before the first save, restores the example. |
...
- **A refused write** (quota, blocked storage) retains a page-only copy.
  **Reopen** loads a usable browser-saved note if one is available; otherwise
  it brings back the page-held copy and says it is session-only. This works
  even when reads succeed but storage is empty after a refused write.
...
reports that plainly without erasing the draft or its local recovery copy.
**Discard draft** can still return to that copy, with a status that makes clear
it is no longer confirmed in browser storage.
...
**可以做什么**：编辑器工具栏自带三个操作 —— **保存**（把标题和正文写入本浏览器）、**重新打开**（把最近保存的副本读回编辑器）、**放弃草稿**（退回最近保存的副本，不写入也不删除任何内容）。字段旁的状态行会如实显示当前是示例、草稿、已保存，还是仅限本次会话。
...
两种失败分别处理：**写入被拒绝**（配额、存储被阻止）时保留页面内副本。“重新打开”始终重试真实存储；有可用的浏览器已存笔记就读取它，否则取回页面内副本并注明“仅限本次会话”，包括写入失败后仍可读取、但存储为空的情况。**没有页面内保存副本时的读取失败**会保留草稿与最近确认的副本，并说明无法读取存储；下次“重新打开”仍可重试。读取失败不能证明没有记录。若确认存储为空（例如其他标签页已清除），示例会如实说明，但仍保留草稿和本地恢复副本；“放弃草稿”可返回该副本，状态不会把它误称为仍在浏览器存储中。
